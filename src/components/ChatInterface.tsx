...
import { IChatMessage } from '../types/scraper';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const result = await scraperAPI.chatWithJob(jobId, userMessage.content);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground opacity-50">
            ...
            <p className="text-center">Ask questions about the scraped content.<br />Example: "Summarize the main points" or "Extract all prices"</p>
          </div>
        ) : (
          messages.map((msg, index) => (
            ...
            >
              ...
              >
                <div
                  className={`px-4 py-3 rounded-lg text-sm ${msg.role === 'user'
                      ? 'bg-primary text-primary-foreground'
